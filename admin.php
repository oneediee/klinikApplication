    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbar-ex-collapse">
          <ul class="nav navbar-nav navbar-right">
            <li class="active"><a href="admin.php"><font face="Lucida Bright" size="+1" color="green"><b>Dokter</b></a></li></font>
...
<div class="container">
    ...
    <table id="myTable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
        <?php include 'proses/dokter/tbl_dkt.php';
        while($d = mysql_fetch_array($sql)){
            ?>
            <tr>
                <td><?php echo $d['kd_dkt']; ?></td>
                <td><?php echo $d['nm_dkt']; ?></td>
                <td><?php echo $d['spesialis']; ?></td>
                <td><?php echo $d['almt_dkt']; ?></td>
                <td><?php echo $d['telp_dkt']; ?></td>
                <td><?php echo $d['tarif']; ?></td>
                <td align="center">
                    <a href="#" class="btn btn-primary modal_edit" id="<?php echo $d['kd_dkt']; ?>">
                        <i class="fa fa-edit"></i> Edit</a></td>
                   <td> <a href="#" class="btn btn-danger modal_delete" id="<?php echo $d['kd_dkt']; ?>">
                ...
                <form id="formModal" action="" method="post">
                    <div class="form-group">
                        <label>Kode Dokter</label>
                            <input type="text" class="form-control" name="kd_dkt" id="kd_dkt" value="<?php include 'proses/dokter/kd_dkt.php';?>" readonly>
                    </div>
                    <div class="form-group">
                        <label>Nama Dokter</label>
                        <input type="text" class="form-control" id="nm_dkt" name="nm_dkt" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="almt_dkt" id="almt_dkt" rows="3" class="form-control" required></textarea>
                    <div class="form-group">
                        <label>No Telepon</label>
                        <input type="number" class="form-control" id="telp_dkt" name="telp_dkt" required>
                    </div>
                    <div class="form-group">
                        <label>Tarif</label>
   ...
   $(document).ready(function () {
        $('#myTable').dataTable({
            ...
            buttons: [  { extend: 'excel', exportOptions: {columns: ':visible'} },
                ...
                { extend: 'colvisGroup', text: 'Data dokter', show: [ 1, 2, 3, 4, 5 ], hide: [ 0,6 ] },
    ...
    function ModalAdd() {
        $('#Mymodal').modal('show');
        $('#btnS').text("Simpan");
        $('#formModal').attr({action: "proses/dokter/tambah.php"});
        $('#formModal')[0].reset();
        $('#kd_dkt')[0].reset();
        $('.modal-title').text(" Add Data Dokter");
    }
    //Edit Data
    $(".modal_edit").click(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $('#tarif').val(json[5]);
                $('#Mymodal').modal('show');
                $('#formModal').attr({action: "proses/dokter/edit_proses.php"});
            }
        });
    })
    //Delete Data
